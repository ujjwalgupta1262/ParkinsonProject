...
def normalise(list):
    ...
    std = numpy.std(list,axis = 0)
    epsilon = numpy.array(numpy.exp(-10))
    return (list - mean)/(std+epsilon)
...
def get_young(list,label):
    index_pd = [5, 7, 11, 21, 22, 23, 25, 26, 30, 31, 34]
    index_hc = [0, 2, 3, 6, 8, 10, 11, 12, 15, 18, 20, 21, 22, 23, 24, 25, 27, 29, 30, 32, 33, 34, 37]
    if (label == "pd"):
...
accuracy_list = []
passed_vec = open("young_passed.txt", "r").readlines()
...
for i in range(len(passed_features)):
    if (passed_features[i] != -100):
        ...
        # print(temp_str)
        temp_pd = temp_str + "_pd_t2.txt"
        # print(temp_pd)
        temp_hc = temp_str + "_hc_t2.txt"
...
# All the Nan values were considered as Zero as essentially their contribbution remains null and void
        ...
        if (flag == 0):
            X = get_young(vec_pd,"pd") + get_young(vec_hc,"hc")

        elif (flag == 1):
            if (column == 5):
                X = get_young(get_column(vec_pd, 0),"pd") + get_young(get_column(vec_hc, 0),"hc")
                X = [x for x in X if math.isnan(x)==False]
            elif (column == 10):
                X = get_young(get_column(vec_pd, 1),"pd") + get_young(get_column(vec_hc, 1),"hc")
                X = [x for x in X if math.isnan(x)==False]
            elif (column == 20):
                X = get_young(get_column(vec_pd, 2),"pd") + get_young(get_column(vec_hc, 2),"hc")
                X = [x for x in X if math.isnan(x)==False]
            elif (column == 30):
                X = get_young(get_column(vec_pd, 3),"pd") + get_young(get_column(vec_hc, 3),"hc")
                X = [x for x in X if math.isnan(x)==False]
            elif (column == 40):
                X = get_young(get_column(vec_pd, 4),"pd") + get_young(get_column(vec_hc, 4),"hc")
                X = [x for x in X if math.isnan(x)==False]
            ...
            if (column == 1):
                ...
                X = get_young(get_column(vec_pd, 5),"pd") + get_young(get_column(vec_hc, 5),"hc")
                X = [x for x in X if math.isnan(x)==False]
            elif (column == 90):
                X = get_young(get_column(vec_pd, 6),"pd") + get_young(get_column(vec_hc, 6),"hc")
                X = [x for x in X if math.isnan(x)==False]
            elif (column == 95):
                X = get_young(get_column(vec_pd, 7),"pd") + get_young(get_column(vec_hc, 7),"hc")
                X = [x for x in X if math.isnan(x)==False]
            elif (column == 99):
                X = get_young(get_column(vec_pd, 8),"pd") + get_young(get_column(vec_hc, 8),"hc")
                X = [x for x in X if math.isnan(x)==False]
        ...
        y = [0] * len(get_young(vec_pd,"pd")) + [1] * len(get_young(vec_hc,"hc"))
