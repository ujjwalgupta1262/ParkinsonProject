...
accuracy_list = []
passed_vec = open("old_passed.txt", "r").readlines()
...
# All the Nan values were considered as Zero as essentially their contribbution remains null and void
        ...
        if (flag == 0):
            X = get_old(vec_pd,"pd") + get_old(vec_hc,"hc")

        elif (flag == 1):
            if (column == 5):
                X = get_old(get_column(vec_pd, 0),"pd") + get_old(get_column(vec_hc, 0),"hc")
                X = [x for x in X if math.isnan(x)==False]
            elif (column == 10):
                X = get_old(get_column(vec_pd, 1),"pd") + get_old(get_column(vec_hc, 1),"hc")
                X = [x for x in X if math.isnan(x)==False]
            elif (column == 20):
                X = get_old(get_column(vec_pd, 2),"pd") + get_old(get_column(vec_hc, 2),"hc")
                X = [x for x in X if math.isnan(x)==False]
            elif (column == 30):
                X = get_old(get_column(vec_pd, 3),"pd") + get_old(get_column(vec_hc, 3),"hc")
                X = [x for x in X if math.isnan(x)==False]
            elif (column == 40):
                X = get_old(get_column(vec_pd, 4),"pd") + get_old(get_column(vec_hc, 4),"hc")
                X = [x for x in X if math.isnan(x)==False]
            ...
            if (column == 1):
                ...
                X = get_old(get_column(vec_pd, 5),"pd") + get_old(get_column(vec_hc, 5),"hc")
                X = [x for x in X if math.isnan(x)==False]
            elif (column == 90):
                X = get_old(get_column(vec_pd, 6),"pd") + get_old(get_column(vec_hc, 6),"hc")
                X = [x for x in X if math.isnan(x)==False]
            elif (column == 95):
                X = get_old(get_column(vec_pd, 7),"pd") + get_old(get_column(vec_hc, 7),"hc")
                X = [x for x in X if math.isnan(x)==False]
            elif (column == 99):
                X = get_old(get_column(vec_pd, 8),"pd") + get_old(get_column(vec_hc, 8),"hc")
                X = [x for x in X if math.isnan(x)==False]
        ...
        y = [0] * len(get_old(vec_pd,"pd")) + [1] * len(get_old(vec_hc,"hc"))
